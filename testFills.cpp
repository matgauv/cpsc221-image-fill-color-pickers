...
TEST_CASE("f") {
    PNG imgtest;
    imgtest.readFromFile(CENSORTESTIMAGE);
    CensorColorPicker censorPicker(CENSORWIDTH, PixelPoint(0 , 0, HSLAPixel()), 100, imgtest);

    filler::FillerConfig censorconfig;
    censorconfig.img = imgtest;
    censorconfig.frameFreq = CENSORFRAMEFREQ;
    censorconfig.tolerance = CENSORTOLERANCE;
    censorconfig.seedpoint = PixelPoint(0, 0, *censorconfig.img.getPixel(0, 0));
    censorconfig.picker = &censorPicker;

    animation anim;
    anim = filler::FillBFS(censorconfig);
    PNG result = anim.write("images/mytest.gif");
    result.writeToFile("images/mytest.png");
