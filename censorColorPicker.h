...
class CensorColorPicker : public ColorPicker
{
  ...
  private:
    ...
    PNG CreateBlockImage();
    HSLAPixel * GetAveragePixel(unsigned int x, unsigned int y);
    HSLAPixel * AverageColor(double sum_h, double sum_s, double sum_l, double sum_a, double block_count);
    void ApplyToWholeBlock(PNG& blocky, HSLAPixel * average_pixel, unsigned int x, unsigned int y);
